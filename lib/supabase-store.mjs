...
export function createSupabaseStore(url, key) {
  ...
  return {
    ...
    async check() {
      ...
      await result(client.from('media').select('instagram_url').limit(0));
    ...
    search: async (filters, visitor) => {
      ...
      return { ...data, items: data.items.map(item => ({ ...item, ...engagement.find(e => e.media_id === item.id), url: bucket.getPublicUrl(item.filename).data.publicUrl })) };
    },
    ...
    setInstagram: (id, instagram_url) => result(client.from('media').update({ instagram_url }).eq('id', id).select('id,instagram_url').maybeSingle()),
