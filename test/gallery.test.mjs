...
test('Supabase schema, SDK, admin, media lifecycle and failure handling', { timeout: 120000 }, async () => {
  ...
  try {
    ...
    discovery.set('title', 'Cafe discovery'); discovery.set('category_id', category.id);
    discovery.set('description', 'Coffee and a quiet courtyard.\nNear the metro.');
    discovery.set('file', new Blob([Buffer.from('00000018667479706d703432000000006d70343269736f6d', 'hex')]), 'clip.mp4');
    discovery.append('photos', new Blob([png]), 'interior.png');
    discovery.append('photos', new Blob([png]), 'menu.png');
    const createdDiscovery = await request('/api/media', { method: 'POST', body: discovery });
    assert.equal(createdDiscovery.status, 201);
    const discoveryId = (await createdDiscovery.json()).id;
    const discoveryItem = (await (await request('/api/media')).json()).items[0];
    assert.equal(discoveryItem.type, 'video');
    assert.equal(discoveryItem.photos.length, 2);
    assert.match(discoveryItem.description, /quiet courtyard/);
    for (const photo of discoveryItem.photos) assert.equal((await fetch(photo.url)).status, 200);
    assert.equal((await request('/api/media/' + discoveryId, { method: 'DELETE' })).status, 200);
    for (const photo of discoveryItem.photos) assert.equal((await fetch(photo.url)).status, 404);
    discovery.set('description', 'x'.repeat(2001));
    assert.equal((await request('/api/media', { method: 'POST', body: discovery })).status, 400);
    discovery.set('description', 'Valid');
    discovery.append('photos', new Blob([Buffer.from('invalid')]), 'fake.png');
    assert.equal((await request('/api/media', { method: 'POST', body: discovery })).status, 400);
    ...
    assert.match((await failedUpload.json()).error, /saving files to Supabase Storage.*Reference:/);
